...
class makeText(object):
    ...
    def makeText(self):
        ...
        breifText = "<최신 보건복지부 브리핑>\n{} : {}\n{} : {}\n".format(self.data["data"]["briefTopics1"][0],self.data["data"]["briefTopics1"][1],self.data["data"]["briefTopics2"][0],self.data["data"]["briefTopics2"][1])
        issuesText = "<최신 핫이슈>\n {} : {}\n{} : {} \n\n\n\n\n\n\n\n데이터 제공 : 대한민국 행정안전부 공공데이터 포털\n서비스 제공자 : Hoplin (https://github.com/J-hoplin1)".format(self.data["data"]["issueTopics1"][0],self.data["data"]["issueTopics1"][1],self.data["data"]["issueTopics2"][0],self.data["data"]["issueTopics2"][1])
        completedText = InfoText + breifText + issuesText
